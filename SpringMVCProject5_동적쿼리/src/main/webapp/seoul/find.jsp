			...
			<div class="text-center">
				<ul class="pagination">
				  <c:if test="${startpage>1 }">
					<li><a href="../seoul/find.do?addr=${addr }&page=${startpage-1 }">&lt;</a></li>
				  </c:if>
					<c:forEach var="i" begin="${startpage }" end="${endpage }">
						<li ${curpage==i?"class=active":"" }><a href="../seoul/find.do?addr=${addr }&page=${i }">${i }</a></li>
					</c:forEach>
				  <c:if test="${endpage<totalpage }">
					<li><a href="../seoul/find.do?addr=${addr }&page=${endpage+1 }">&gt;</a></li>
				  </c:if>
				</ul>
			</div>
		</div>
